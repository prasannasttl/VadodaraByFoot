...
namespace VadodaraByFoot.View.MenuPages
{
	...
	public partial class Contact_Us : ContentPage
	{
		...
		public async System.Threading.Tasks.Task CallWebserviceFeedback()
		{
			if (AppStatics.CheckInternetConnection())
			{
				try
				{
                    ...
                    var response = await CallWebservice.GetResponse_Post<RootObjectContactUs>(WebServiceURL.Url_ContactUs, values);
		...
		{
            if (await this.DisplayAlert(AppResources.AppResources.LDialNumber, AppResources.AppResources.LCall + lblOfficePhoneNumber.Text + "?",AppResources.AppResources.LYes,AppResources.AppResources.LNo))
			{
				var dialer = DependencyService.Get<IDialer>();
				if (dialer != null)
				{
                    dialer.Dial(lblOfficePhoneNumber.Text.Trim());
				}
			}
		}
		public void OnEmailTapped(object sender, EventArgs e)
		{
            ...
            string EmailId = "mailto:" + lblOfficeEmail.Text;
